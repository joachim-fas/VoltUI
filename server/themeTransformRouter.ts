...
export function handlePreview(req: any, res: any) {
  ...
  if (!cached) {
    return res.status(404).send("<html><body style='font-family:sans-serif;padding:24px'><p>Vorschau nicht mehr verfügbar. Bitte erneut transformieren.</p></body></html>");
  }
  ...
  if (!file) {
    return res.status(404).send("<html><body style='font-family:sans-serif;padding:24px'><p>Datei nicht gefunden.</p></body></html>");
  }

  // Wenn LLM-generiertes HTML verfügbar: direkt senden
  if (file.htmlPreview) {
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    return res.send(file.htmlPreview);
  }

  // Fallback: Hinweis dass Vorschau noch generiert wird
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.send(`<!DOCTYPE html><html lang="de"><head><meta charset="UTF-8"><title>Volt UI Vorschau</title><style>body{font-family:'DM Sans',sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#F5F5F5;color:#6B6B6B;}</style></head><body><div style="text-align:center"><p style="font-size:14px">Vorschau wird beim nächsten Transformations-Durchlauf generiert.</p><p style="font-size:12px;margin-top:8px;font-family:monospace">${file.path}</p></div></body></html>`);
}
