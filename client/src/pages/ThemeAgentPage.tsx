 ...
 * Eigenes Layout mit schwarzer Kopfzeile und Zurück-Navigation.
 */
...
function StackBadge({ stack }: { stack: TechStack }) {
  const labels: Record<TechStack, { label: string; color: string }> = {
    "react-tailwind": { label: "React + Tailwind", color: "bg-white text-[#0A0A0A]" },
...
function LogTerminal({ lines }: { lines: string[] }) {
  return (
    <div className="bg-black border border-white/10 rounded-lg p-4 font-mono text-xs max-h-48 overflow-y-auto">
      {lines.map((line, i) => (
    ...
    <div className="border border-white/10 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-mono text-xs text-white/70 truncate">{file.path}</span>
          {hasChanges && (
            <span className="flex-shrink-0 inline-flex items-center gap-1 px-2 py-0.5 bg-white/10 text-white/80 rounded text-xs font-medium">
      ...
      {open && (
        <div className="p-4 space-y-3 bg-black/30">
          {hasChanges && (
            <div className="flex flex-wrap gap-2">
              {file.changes.map((change, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-white/5 border border-white/10 rounded text-xs font-mono text-white/60">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-xs font-mono text-white/30 mb-1.5 flex items-center gap-1">
            ...
            <div>
              ...
              <pre className="bg-white/5 border border-white/10 rounded p-3 text-xs font-mono overflow-x-auto max-h-48 whitespace-pre-wrap break-all">
                <code className="text-white/70">{file.transformedContent}</code>
              </pre>
            </div>
          </div>
        </div>
    ...
    <div className="border border-white/10 rounded-xl overflow-hidden">
      <div className="px-4 py-3 bg-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white/80">GitHub Token</span>
          <span className="text-xs text-white/30">(für private Repositories)</span>
        </div>
        {tokenData?.hasToken && (
          <div className="flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-white/60" />
            <span className="text-xs font-mono text-white/60 font-semibold">Aktiv</span>
          </div>
        )}
      </div>
      ...
      <div className="p-4 space-y-3">
        {tokenData?.hasToken ? (
          <div className="flex items-center justify-between gap-3 bg-white/5 border border-white/10 rounded-lg px-4 py-3">
            <div className="min-w-0">
              <div className="text-xs text-white/30 mb-0.5">{tokenData.label ?? "GitHub Token"}</div>
              <code className="text-sm font-mono text-white/60">{tokenData.maskedToken}</code>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                onClick={() => setShowInput(true)}
                className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-lg border border-white/10 text-white/40 hover:text-white/70 hover:bg-white/5 transition-colors"
              >
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-10 py-2 border border-white/10 rounded-lg bg-white/5 text-white/80 text-sm font-mono placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-white/20"
              />
              <button
                type="button"
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-white/10 rounded-lg bg-white/5 text-white/80 text-sm placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-white/20"
            />
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-white text-[#0A0A0A] rounded-lg text-xs font-semibold hover:bg-white/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <span className="text-xs text-white/20">Token wird gehasht – nie im Klartext gespeichert.</span>
            </div>
          </div>
        )}
        ...
        <p className="text-xs text-white/25">
          ...
          mit <code className="font-mono bg-white/5 px-1 rounded">repo</code>-Berechtigung.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-white/40 text-sm font-mono">{result.owner}/</span>
          ...
          <button
            onClick={onDownload}
            className="flex items-center gap-2 px-4 py-2.5 bg-white text-[#0A0A0A] rounded-lg font-semibold text-sm hover:bg-white/90 transition-colors flex-shrink-0"
          >
      ...
      <div className="border border-white/10 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-white/40">
          ...
          <code className="font-mono text-xs bg-white/5 px-1 py-0.5 rounded">volt-ui.css</code> und eine{" "}
          <code className="font-mono text-xs bg-white/5 px-1 py-0.5 rounded">VOLT_UI_MIGRATION.md</code>{" "}
        ...
        <div className="max-w-4xl mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm text-white/80">Theme Agent</span>
              <span className="text-[0.55rem] font-mono text-white/25 bg-white/5 px-1.5 py-0.5 rounded uppercase tracking-widest">V2</span>
            </div>
          </div>
          <a
            href="https://github.com/settings/tokens"
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-mono text-white/25 uppercase tracking-widest">&gt;_ volt ui · theme agent</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={i} className="border border-white/10 rounded-lg p-4 bg-white/3">
              <div className="text-white/40 mb-2">{f.icon}</div>
              <div className="font-semibold text-sm text-white/70 mb-0.5">{f.title}</div>
        ...
        <div className="border border-white/10 rounded-xl p-6 space-y-4">
          <h3 className="font-semibold text-white/80 flex items-center gap-2 text-sm">
            ...
            {tokenData?.hasToken && (
              <span className="flex items-center gap-1 text-xs font-normal text-white/40 bg-white/5 px-2 py-0.5 rounded-full">
          ...
          <div className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-white/10 rounded-lg bg-white/5 text-white text-sm font-mono placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-white/20"
                disabled={isLoading}
              />
            </div>
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-white text-[#0A0A0A] rounded-lg font-semibold text-sm hover:bg-white/90 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              {isLoading
        ...
        {/* Lade-Zustand */}
        {isLoading && (
          <div className="border border-white/10 rounded-xl p-10 flex flex-col items-center gap-4 text-center">
            <div className="relative">
              <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center">
                <Sparkles className="w-6 h-6 text-white/50" />
              </div>
              <div className="absolute -inset-1 rounded-xl border border-white/10 animate-pulse" />
            </div>
            <div>
              <div className="font-semibold text-white/70 mb-1">Transformation läuft...</div>
        ...
        {!result && !isLoading && !error && (
          <div className="border border-dashed border-white/10 rounded-xl p-14 flex flex-col items-center gap-3 text-center">
            <Github className="w-10 h-10 text-white/15" />
            <div className="font-semibold text-white/30">Bereit zur Transformation</div>
            <div className="text-sm text-white/20 max-w-sm">
