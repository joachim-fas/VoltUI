      ...
      <div className="flex h-[calc(100vh-56px)] overflow-hidden bg-background">
        ...
        <aside className="hidden lg:flex flex-col w-60 flex-shrink-0 border-r border-border bg-card">
          ...
          <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
            <p className="text-[0.65rem] font-semibold text-muted-foreground uppercase tracking-widest px-3 py-2 mt-1">
              Hauptmenü
            </p>
            {NAV_ITEMS.map((item) => (
              <button
          ...
          <div className="p-3 border-t border-border">
            <div className="flex items-center gap-2.5 px-2 py-2">
              ...
              <LogOut className="w-4 h-4 text-muted-foreground flex-shrink-0 cursor-pointer hover:text-foreground transition-colors" />
            </div>
          </div>
        </aside>
          ...
          <header className="flex items-center justify-between px-6 h-14 border-b border-border bg-background flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <div className="hidden md:block">
                <VoltCommandBar
                  placeholder="Suchen …"
                  onSubmit={(v) => add({ title: `Suche: "${v}"`, variant: "info" })}
                  className="w-56"
                />
              </div>
      ...
      <VoltModal
        ...
        footer={
          <div className="flex gap-2 justify-end">
            <VoltButton variant="ghost" size="sm" onClick={() => setShowModal(false)}>Abbrechen</VoltButton>
            <VoltButton variant="primary" size="sm" onClick={() => { setShowModal(false); add({ title: "Bericht erstellt", description: reportName || "Unbenannter Bericht", variant: "success" }); }}>Erstellen</VoltButton>
          </div>
        }
      >
        <div className="space-y-4">
          <VoltInput
            label="Berichtsname"
            placeholder="z. B. Q2 Umsatzanalyse"
            value={reportName}
